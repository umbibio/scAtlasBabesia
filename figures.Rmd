...
sync.sc <- readRDS('../Input/compScBabesia/RData_new/sync.sc.rds')


# plot func
overlaySyncScPlot <- function (d, xts = NULL, yts = NULL, offset = 0, main = main,  xlab = xlab, ylab = ylab,
                          match.indices = NULL, match.col = "gray40", match.lty = 4) 
{
  ...
  matplot(cbind(xts, ytso), type = 'l', pch = 21, xlab = xlab, 
          ylab = ylab, axes = FALSE, main = main, lwd = 4, cex.main = 2.2,  cex.lab = 1.8, 
          font = 2, font.lab = 2)
  axis(1, cex.axis = 1.6,  font = 2)
  axis(2, at = pretty(xts), cex.axis = 1.6, font = 2)
  if (offset != 0) {
  ...
  segments(d$index1[idx], xts[d$index1[idx]], d$index2[idx], 
           ytso[d$index2[idx]], col = match.col, lty = match.lty, lwd = 4)
  par(def.par)
}

gene.peak.good <- 'Bdiv_033720'
gen.peak.poor <- 'Bdiv_024320' 
gene.dip.good <- 'Bdiv_040000' 
gene.dip.poor <- 'Bdiv_019820c' 


my.genes <- c(gene.peak.good, gen.peak.poor, gene.dip.good, gene.dip.poor)

##
my.gene.expr <- sync.sc %>% dplyr::filter(GeneID == my.genes[1])
dd <- dtw(my.gene.expr$scaled.expr.sc, my.gene.expr$scaled.expr.sync, k = TRUE, step.pattern = symmetric2)
overlaySyncScPlot(dd, offset =  0, match.indices = 20, main = my.genes[1], xlab = '', ylab = '')

##
my.gene.expr <- sync.sc %>% dplyr::filter(GeneID == my.genes[2])
dd <- dtw(my.gene.expr$scaled.expr.sc, my.gene.expr$scaled.expr.sync, k = TRUE, step.pattern = symmetric2)
overlaySyncScPlot(dd, offset =  0, match.indices = 20, main = my.genes[2], xlab = '', ylab = '')

##
my.gene.expr <- sync.sc %>% dplyr::filter(GeneID == my.genes[3])
dd <- dtw(my.gene.expr$scaled.expr.sc, my.gene.expr$scaled.expr.sync, k = TRUE, step.pattern = symmetric2)
overlaySyncScPlot(dd, offset =  0, match.indices = 20, main = my.genes[3], xlab = '', ylab = '')

##
my.gene.expr <- sync.sc %>% dplyr::filter(GeneID == my.genes[4])
dd <- dtw(my.gene.expr$scaled.expr.sc, my.gene.expr$scaled.expr.sync, k = TRUE, step.pattern = symmetric2)
overlaySyncScPlot(dd, offset =  0, match.indices = 20, main = my.genes[4], xlab = '', ylab = '')

legend(x = "bottomright",          # Position
       legend = c("scRNA-Seq", "Bulk"),  # Legend texts
...
sync.sc <- readRDS('../Input/compScBabesia/RData_new/sync.sc.rds')
peaks   <- readRDS('../Input/compScBabesia/RData_new/peaks.rds')
dwt.dist <- readRDS('../Input/compScBabesia/RData_new/dwt.dist.rds')

sync.sc.means <- sync.sc %>% dplyr::select(GeneID, mean.expr.sc, mean.expr.sync) %>% distinct()
sm.peaks <- peaks %>% dplyr::filter(Phase %in% c('SM', 'MC'))
non.sm.peaks <- peaks %>% dplyr::filter(!(Phase %in% c('SM', 'MC')))
sm.boundary <- c(2, 11)
peaks <- peaks %>% mutate(smc.peak = ifelse(Phase %in% c('SM', 'MC') & 
                                              peak.sc > sm.boundary[1] & 
                                              peak.sc < sm.boundary[2], 'Yes', 'No'), 
                          smc.dip = ifelse(!(Phase %in% c('SM', 'MC')) & 
                                             dip.sc > sm.boundary[1] & 
                                             dip.sc < sm.boundary[2], 'Yes', 'No'))
dist.genes <- dwt.dist$GeneID[dwt.dist$d >= quantile(dwt.dist$d, probs = 0.8)]
close.genes <- dwt.dist$GeneID[dwt.dist$d <= quantile(dwt.dist$d, probs = 0.2)]
peaks$Alignment <- 'Mid'
peaks$Alignment[peaks$GeneID %in% dist.genes] <- 'Low'
peaks$Alignment[peaks$GeneID %in% close.genes] <- 'High'
peaks$Alignment <- factor(peaks$Alignment, levels = c('Low', 'Mid', 'High'))


stats.align <- peaks %>% group_by(Phase, Alignment) %>% summarise(total = n(), percent = n()) %>% 
  group_by(Phase) %>% mutate(percent=percent/sum(percent)*100, 
                             ylab_pos = ceiling((105 - cumsum(percent))))

ind <- which(stats.align$total < 3)
stats.align$total <- as.character(stats.align$total)
stats.align$total[ind] <- ''


p <- ggplot(stats.align, aes(fill=Alignment, y=percent, x=Phase)) + 
  geom_bar(position="stack", stat="identity") + 
  geom_text(aes(y=ylab_pos, label=total), vjust=0.5, color="black", size=8, fontface = 'bold') + 
  theme_minimal() + 
  theme(
...
peaks$Phase <- as.character(peaks$Phase)
peaks$Phase[is.na(peaks$Phase)] <- 'NoPeak'
peaks$Phase <- factor(peaks$Phase, levels = c('G', 'SM', 'MC', 'C', 'NoPeak'))

peaks.filt <- peaks %>% dplyr::filter(smc.peak == 'Yes' & peak.sync > 0 & peak.sync < 12)
cor.txt <- cor(peaks.filt$peak.sc, peaks.filt$peak.sync)

p2 <- ggplot(peaks.filt, aes(x = peak.sc, y = peak.sync)) + 
  geom_point(aes(
    fill = mean.expr.sc,
    #color = aligns
  ), #color = 'blue', 
  ...
  ylab("Bulk") + xlab("Single Cell") + theme_minimal() +
  # annotate(cor.txt,
  #   size    = 4,
  #   fontface="bold",
  #   hjust   = 1.05,
  ...
  # ) +
  #facet_wrap(.~Phase)+
  #ggtitle("Peak time correlation") + 
  scale_fill_continuous(type = "viridis") + 
  annotate(geom="text", x=6.5, y=11.5, label=paste(round(cor.txt, 3)), color="black",size=8, fontface = "bold.italic") + 
  #annotate(geom="text", x=1.4, y=11.1, label="P-Value < 0.0001", color="black",size=4, fontface = "bold.italic") +
...
cutoff <- quantile(peaks$d, probs = c(0.2, 0.8))

hist.y <- density(peaks$d, from = 0, to = 1)
hist.y <- data.frame(x = hist.y$x, y = hist.y$y) %>% 
  #mutate(aligarea1 = x <= cutoff[1], area2 = (x>=cutoff[1] & x <= cutoff[2]), area3 = x >= cutoff[2]) %>%
  mutate(aligns = ifelse(x <= cutoff[1], 'well', ifelse(x>=cutoff[1] & x <= cutoff[2], 'mid',' poor')))

p3 <- ggplot(data = hist.y, aes(x = x, ymin = -0.1, ymax = y, fill = aligns)) +
  geom_ribbon(alpha = 0.9) +
  geom_line(aes(y = y), size = 1.2) +  
  geom_vline(aes(xintercept=cutoff[1]),
             color="black", linetype="dashed", size=0.8) +
  geom_vline(aes(xintercept=cutoff[2]),
             color="black", linetype="dashed", size=0.8) +
  ylab("density") + xlab("dtw distance") + theme_minimal() +
...
## figure 7
## Please check the web application (https://umbibio.math.umb.edu/babesiasc/) for the network figure 7C.
...
cluster.dtw <- paste("cluster", 1:2, sep = " ")
...
p2 <- do.call(grid.arrange, c(p, nrow=1))
...
## Figure 8 A
## Please check web application (https://umbibio.math.umb.edu/babesiasc/) for the network figure 8C.
...
sc.tc.mus.long.all <- read.xlsx("./rds/AP2s_TFs_3genes_added_regulators_dtw_clusters.xlsx")
genes <- c("Bdiv_031830", "Bdiv_024900c", "Bdiv_015020c")
sc.tc.mus.long.all.subset <- sc.tc.mus.long.all %>% filter(GeneID %in% genes) 
sc.tc.mus.long.all.subset$GeneID <- gsub("Bdiv_", "", sc.tc.mus.long.all.subset$GeneID)

df.plot <- sc.tc.mus.long.all.subset
df.plot$cluster <- 1
...
p <- lapply(unique(df.plot$cluster), function(i){
  ...
  ggplot(tmp, aes(x = t, y = y, color = GeneID)) + 
    ...
    theme(
      strip.text = element_blank(),
      strip.background = element_rect(fill = "white", colour = "black"), 
      strip.text.y  = element_text(size = 6, face = 'bold',  color = "black"),
      ...
      axis.text.x = element_text(size = 20, color = "black", angle = 90,face = "bold"),
      axis.text.y = element_text(size = 20, color = "black", angle = 0, face = "bold"),
      #axis.ticks.x = element_blank()
    ) +
    theme(strip.text.y = element_text(margin = margin(2,1,2,1, "cm")),
          panel.spacing = unit(0.75, "lines"))+
...
## SI figure 1 left panel

```{r}


S.Os <- readRDS('./rds/S_O_list_individual_processed_matched_clusts.rds')

## Plot with new cluster assignment
ps <- lapply(S.Os, function(S.O){
  Idents(S.O) <- 'ordered.clust'
  p <- DimPlot(S.O, reduction = "umap", 
               #group.by = "cell", 
               #split.by = 'spp',
               pt.size = 0.8,
               #shape.by='spp',
               label = F, repel = T, label.size = 5) + NoLegend() +
    ylab('UMAP2') + xlab('UMAP1') +
...
## SI figure 1 right panel

```{r}
orthologs <- read.xlsx("./rds/Bdiv_Bmic_Bbov_Bbig_orth.xlsx")
toxo.orth <- read.xlsx("./rds/GT1_BDiv.xlsx")

all.markers.sig <- lapply(S.Os, function(S.O){
  Idents(S.O) <- 're.clust'
  markers <- FindAllMarkers(object = S.O, only.pos = T, min.pct = 0)
  
  markers$GeneID <- gsub('-', '_', markers$gene)
  if(S.O@meta.data$spp[1] == 'BBig'){
    match.id <- match(markers$GeneID, orthologs$Bbig)
  }else if(S.O@meta.data$spp[1] == 'BBov'){
    match.id <- match(markers$GeneID, orthologs$Bbov)
    
  }else{
    match.id <- match(markers$GeneID, orthologs$Bdiv)
  }
  markers$orth <- orthologs$Bdiv[match.id]
  markers$spp <- S.O@meta.data$spp[1]
  markers.sig <- markers %>% dplyr::filter(avg_log2FC > log2(2) & p_val_adj < 0.01)
  markers.sig <- left_join(markers.sig, toxo.orth, by = c('orth' = 'GeneID'))
  markers.sig
})


## Cell cycle Markers
all.markers.sig.bar <- bind_rows(all.markers.sig)


ss <- all.markers.sig.bar  %>% group_by(spp, cluster) %>% summarise(num.DEG = n())
ss$ordered.clust <- ifelse(ss$cluster == 0, 'C0',
                           ifelse(ss$cluster == 3, 'C1',
                                  ifelse(ss$cluster == 1, 'C2', 'C3')))
ss$ordered.clust <- factor(ss$ordered.clust, levels = c("C0", "C1", "C2", "C3"))

ss$spp <- factor(ss$spp, levels = c("BBig", "BBov", "BDiv_Cow", "BDiv_Human"))
p <- ggplot(data=ss, aes(x=ordered.clust, y=num.DEG, fill=ordered.clust)) +
...
## SI figure 4B top panel (T.gondii)
```{r}
...
sc.tc.mus.long.all <- read.xlsx("./rds/DNA_Replicating_genes_G_Phase.xlsx")
...
p <- lapply(unique(df.plot$cluster), function(i){
  ...
  ggplot(tmp, aes(x = t, y = y, color = GeneID)) + 
    ...
    theme(
      ...
      strip.background = element_rect(fill = "white", colour = "black"), 
     # strip.text.y  = element_text(size = 13, face = 'bold',  color = "white"),
      plot.title = element_text(size = 24, face = "bold.italic", hjust = 0.5), 
      ...
      axis.text.x = element_text(size = 10, color = "black", angle = 90),
      axis.text.y = element_text(size = 0, color = "black", angle = 0),
      #axis.ticks.x = element_blank()
...
## SI figure 9
## Please check the web application (https://umbibio.math.umb.edu/babesiasc/) for the expression of genes.
...
## Figure SI 11
## Please check the expression of PKG on web application (https://umbibio.math.umb.edu/babesiasc/). 


## Figure SI 12 
## Please check the expression of Vesa genes on web application (https://umbibio.math.umb.edu/babesiasc/).
